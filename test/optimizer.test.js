...
describe("The optimizer", () => {
  ...
  describe("constant folds comparisons", () => {
    it("folds 3n < 5n → true", () => {
      folds(core.binary("<", 3n, 5n, core.boolType), true)
    })

    it("folds 5n < 3n → false", () => {
      folds(core.binary("<", 5n, 3n, core.boolType), false)
    })
  ...
  describe("strength reductions (left operand is constant)", () => {
    ...
    it("folds 0n - x → -x", () => {
      assert.deepEqual(
        optimize(core.binary("-", 0n, x, core.intType)),
        core.unary("-", x, core.intType)
      )
    })
  })
